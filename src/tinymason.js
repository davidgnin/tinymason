(function tinymason(w) {
  ...
  w.tinymason = function tinymason(params) {
    var resize, prepare, fillGrid, genClassName, //Private methods
      ...
      elements = [],
      columnsNum = Infinity,
      prefix = params.prefix ? params.prefix : 'tinymason',
    ...
    fillGrid = function tinymason_fillGrid(fromElement) {
      ...
      for (i = fromElement, length = elements.length; i < length; i += 1) {
        bestHeight = Infinity;
    ...
    genClassName = function tinymason_genClassName(name, num) {
      ...
      return name ? className + ' ' + prefix + '-' + name + '-' + num
        : className;
    };
